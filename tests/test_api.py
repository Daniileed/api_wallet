...
async def test_operation_type_unprocessable_content_422(client, wallet_id):
    response = await client.post(
        f"/api/v1/wallets/{wallet_id}/operation",
        json={"operation_type": "", "amount": 10000}
    )
    assert response.status_code == 422

async def test_amount_unprocessable_content_422(client, wallet_id):
    response = await client.post(
        f"/api/v1/wallets/{wallet_id}/operation",
